...
# Get the original true video dimensions
ORIG_H, ORIG_W = original_frame.shape[:2]

# Target display dimensions so it fits comfortably on your monitor
DISPLAY_W, DISPLAY_H = 1280, 720
display_frame = cv2.resize(original_frame, (DISPLAY_W, DISPLAY_H))

# Store our two clicks
clicked_points = []
...
def click_line_event(event, x, y, flags, params):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if len(clicked_points) < 2:
            # Scale the clicked coordinates BACK up to the true 1920x1080 resolution
            ...
            # Draw visual feedback on the resized screen
            cv2.circle(display_frame, (x, y), 6, (0, 0, 255), -1)
            cv2.putText(display_frame, f"P{len(clicked_points)} ({true_x},{true_y})", (x + 10, y - 10), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
            
            # If we have both points, draw the complete imaginary line across the road
            if len(clicked_points) == 2:
                # Calculate the display equivalents to draw the line visually
